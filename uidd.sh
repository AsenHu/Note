...
then
    if [ "$debian_netmode" == DHCP ]
    then
        tmp=$(echo "apt install curl -y && bash <(curl -L -q --retry 5 --retry-delay 10 --retry-max-time 60 'https://raw.githubusercontent.com/AsenHu/Note/main/debianSet.sh') '$name' '$pass' bookworm $debian_sources $debian_secSources $CFIP '$key' $port $debian_netmode $debian_DHCP_IPv4 $debian_DHCP_IPv6 '$debian_DNS'" |base64 |tr -d "\n")
...
if [ "$system" == ubuntu ]
then
    tmp=$(echo "apt install curl -y && bash <(curl -L -q --retry 5 --retry-delay 10 --retry-max-time 60 'https://raw.githubusercontent.com/AsenHu/Note/main/ubuntuSet.sh') '$name' '$pass' focal $ubuntu_sources $CFIP '$key' $port" |base64 |tr -d "\n")
