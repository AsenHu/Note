...
if [ "$system" == ubuntu ]
then
    bash <(wget --no-check-certificate -qO- 'https://raw.githubusercontent.com/MoeClub/Note/master/InstallNET.sh') -u 20.04 -v 64 -a --mirror "http://$ubuntu_sources/ubuntu/" -cmd "$(echo "bash <(wget --no-check-certificate -qO- 'https://raw.githubusercontent.com/AsenHu/Note/main/ubuntuSet.sh') $name $pass focal $ubuntu_sources $CFIP '$key' $port" |base64 |tr -d "\n")"
fi
